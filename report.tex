...
\title{
  \includegraphics[width=0.25\textwidth]{C:/Users/terez/Documents/LatexProjects/Crypto_Solutions/images/UNIPI.png}\\[3em]
...
crime scene. Last but not least, modeling nonlinear decision boundaries significantly improved classification performance. \\[1em]
...
In the current project we were asked to identify the most likely killer for each crime incident in the ``Piraeus Vice'' data set. 
...
Τhe csv file we were given consists of a total of 4.800 rows, each one representing one crime incident. Also, it consists of 15 columns, one for each feature.
...
Note to teacher: As you may have noticed if you have run our code, on the terminal also appear the Q3's results. Everything on terminal is exclusively 
